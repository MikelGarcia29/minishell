...
/*   Updated: 2022/01/19 18:05:59 by lugonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_quote_extra(char *quote_1, char *quote_2)
{
	if (*quote_1 == 1)
		*quote_2 = 0;
	*quote_1 = 0;
...
extern char	*ft_quote_case(t_prompt *p, char *str)
{
	...
	while (str[++j])
	{
		if (!squote && str[j] == '\'')
			...
			ft_quote_extra(&squote, &dquote);
		else if (dquote && str[j] == '\"')
			ft_quote_extra(&dquote, &squote);
	}
	return (str);
}
